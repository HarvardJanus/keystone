...
class QueryRuleSuite extends FunSuite with Logging {
  test("Collapse Query Rule HToL Test"){
    ...
    val rule = CollapseForwardQueryRule(SubSpace(m1), SubSpace(v1), 1, keys)
  ...
  test("Collapse Query Rule LToH Test"){
    ...
    val rule = CollapseForwardQueryRule(SubSpace(v1), SubSpace(m1), 1, keys)
